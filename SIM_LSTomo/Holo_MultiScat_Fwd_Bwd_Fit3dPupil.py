...
for iangle in range(Nrot):
    ...
    my3dfield = scipy.ndimage.interpolation.rotate(my3dfield, rot_angle, mode='nearest', axes=(0, 1), reshape=False)
    ...
    myallsamples.append(nip.abssqr(my3dfield[:,:,:]))
    allImages+=my3dfield

    viewer.add_image(nip.abssqr(my3dfield[Nz//2,:,:]))



viewer.add_image(np.abs(allImages))
...
for iangle in range(Nrot):
    rot_angle=iangle/Nrot*dAngle
    print(rot_angle)
    aproj = nip.ft(myallsamples[iangle])
